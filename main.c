...
#include <string.h>
 ...
 void AjouterAuto(){
     ...
     strcpy(list[c].identifiant ,"Mathetique");
     list[c].note = List[c].NoteGenerale;
     c++;
 ...
 void Ajouter(){
            ...
            printf("           =====>>  Pour le departement Mathetique taper --- : 2\n");
...
void afficher(){
 ...
 void Statistiques(){
     ...
     for(int k = 0 ; k <= c ; k++ ){
            ...
            if(strcmp(list[k].identifiant,"Mathetique")==0){g++;}
     ...
     printf("            |=====>>  le nombre d'etudiants dans le departement Mathetique  ------------- : %d\n",g);
     ...
     printf("Informatique%.2f",somme);
     for( int k2 = 0 ; k2 <= c ; k2++ ){

            if(strcmp(list[k2].identifiant,"Mathetique")==0){
